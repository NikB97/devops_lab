...
class monitoring:
	...
	def creating_json(self):
		self.metrics()
		self.counter += 1
		data = { "SNAPSHOT:" : str(self.counter), "DATE" : time.ctime(), "cpu percent" : self.cpu_per, "cpu times" : self.cpu_t, "virtual memory" : self.virt_mem, "swap memory" :  self.swap_mem, "disk partitions":  self.disk_parts, "disk usage":  self.disk_use, "net addres" : self.net_addr }
